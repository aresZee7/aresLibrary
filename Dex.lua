		...
		local function main()
			...
			Lib.Signal = (function()
				...
				funcs.Fire = function(self,...)
					for i,v in next,self.Connections or{} do
			...
			--[[newFrame.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.Touch then
					...
					local startIdx = self.Index
					
					input.Changed:Connect(function()
						if input.State == Enum.UserInputState.End then
							local endPos = input.Position
							local delta = endPos - startPos
							local sensitivity = 20
							if delta.Y > sensitivity then
								self:ScrollTo(self.Index - self.WheelIncrement)
							elseif delta.Y < -sensitivity then
								self:ScrollTo(self.Index + self.WheelIncrement)
							end
						end
					end)
			...
			rightSide.Frame:TweenPosition(rightTweenPos,Enum.EasingDirection.Out,Enum.EasingStyle.Quad,0.3,true)]]
				...
				static.ToggleSide = function(name)
					...
					for i,v in pairs(side.Windows or {}) do
						if side.Hidden then
							v.OnDeactivate:Fire()
						else
							v.OnActivate:Fire()
...
Main = (function()
	...
	Main.CreateMainGui = function()
		...
		Main.CreateApp({Name = "Simple Spy", IconMap = Main.LargeIcons, Icon = "Script_Viewer", OnClick = function(callback)
			loadstring(game:HttpGet('https://raw.githubusercontent.com/KeneiUmaru/DeltaHub/simpleSpy.lua'))()
		end})
